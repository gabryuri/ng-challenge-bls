...
import os
from backend.scripts.configuration.configuration import ConfigurationHelper, load_required_env
...
class TestConfiguration(unittest.TestCase):
    ...
    def test_should_fail_when_missing_env_var(self):
        with self.assertRaises(ConfigurationHelper.NecessaryParameterMissing):
    ...
    def test_should_create_database_uri_correctly(self):
        config_helper = ConfigurationHelper(self.logger)
    ...
    def test_configuration_should_fail_when_missing_env_vars(self):
        with self.assertRaises(ConfigurationHelper.NecessaryParameterMissing):
            ConfigurationHelper(self.logger)
